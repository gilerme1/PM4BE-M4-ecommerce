...
const usuarioMock = {
    ...
    password: 'Password123!',       // En el entorno real, las contraseñas están hasheadas.
...
describe('UsersService - test de funcionalidades del servicio de usuarios', () => {
    ...
    let repoMock: Partial<UsersRepository>;  // permite implementar solo los métodos necesarios para las pruebas.

    beforeEach(() => {
         // Cada método simula el comportamiento esperado del repositorio de forma simplificada.
        repoMock = {
        getUsers: async () => [{ ...usuarioMock }],
        getById: async (id: string) => (id === usuarioMock .id ? { ...usuarioMock } : null),
        createUser: async (dto: CreateUserDto) => ({ id: 'uuid-123', email: dto.email }),
        updateUser: async (id: string, dto: Partial<CreateUserDto>) => (id === usuarioMock.id ? id : null),
        deleteUser: async (id: string) => (id === usuarioMock.id ? id : null),
        };

        // 🧪 Inyección del repo de prueba en el servicio
        service = new UsersService(repoMock as UsersRepository);
    });

    it('getUsers() debe retornar un array de usuarios', async () => {
        const users = await service.getUsers();
        expect(users).toHaveLength(1);
        expect(users[0].email).toBe(usuarioMock.email);
    });
